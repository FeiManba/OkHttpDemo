...
//    private IBatchListener mIBatchListener;
...
//     *
//     * @param point
//     * @param IBatchListener
//     */
//    DownloadTask(DownloadData point, IBatchListener IBatchListener) {
//        this.mPoint = point;
//        mIBatchListener = IBatchListener;
//        this.mProgress = new long[THREAD_COUNT];
//        this.mCacheFiles = new File[THREAD_COUNT];
//        this.mHttpUtil = HttpUtil.getInstance();
//    }
//
//    public DownloadTask(String batchId, DownloadData point, IBatchListener IBatchListener) {
...
//                }
//                mPoint.setProgress(progress * 1.0f / mFileLength);
//                mPoint.setCacheType(CacheType.CACHE_TYPE_PROGRESS);
//                mIBatchListener.onProgress(mPoint);
//                break;
//            case MSG_PAUSE://暂停
//                childPauseCount++;
//                if (childPauseCount % THREAD_COUNT != 0) return;
//                resetStutus();
//                mPoint.setCacheType(CacheType.CACHE_TYPE_PAUSE);
//                mIBatchListener.onPause(mPoint);
//                break;
//            case MSG_FINISH://完成
//                childFinshCount++;
//                if (childFinshCount % THREAD_COUNT != 0) return;
//                mTmpFile.renameTo(new File(mPoint.getCacheFilePath(), mPoint.getCacheFileName()));//下载完毕后，重命名目标文件名
//                resetStutus();
//                mPoint.setCacheType(CacheType.CACHE_TYPE_FINISHED);
//                mIBatchListener.onFinished(mPoint);
//                if (!TextUtils.isEmpty(batchId) && mIBatchListener != null) {
//                    mIBatchListener.batchTaskFinished(mPoint);
//                }
//                break;
//            case MSG_CANCEL://取消
//                childCanleCount++;
...
//                mIBatchListener.onCancel(mPoint);
//                break;
//        }
//    }
//
...
//                    mTmpFile = new File(mPoint.getCacheFilePath(), mPoint.getCacheFileName() + ".tmp");
//                    if (!mTmpFile.getParentFile().exists()) mTmpFile.getParentFile().mkdirs();
//                    RandomAccessFile tmpAccessFile = new RandomAccessFile(mTmpFile, "rwd");
...
//        // 分段请求网络连接,分段将文件保存到本地.
//        // 加载下载位置缓存文件
//        final File cacheFile = new File(mPoint.getCacheFilePath(), "thread" + threadId + "_" + mPoint.getCacheFileName() + ".cache");
...
//            @Override
//            public void onResponse(Call call, Response response) throws IOException {
//                if (mPoint.getFileType() == 0 || mPoint.getFileType() == 1) {
//                    if (response.code() != 200) {// 206：请求部分资源成功码
//                        resetStutus();
//                        return;
//                    }
//                } else if (mPoint.getFileType() == 2) {
